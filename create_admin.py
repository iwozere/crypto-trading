...
with app.app_context():
    ...
    if user:
        ...
        update = input('Do you want to update this user to admin and/or reset password? (y/N): ').strip().lower()
        if update == 'y':
            user.is_admin = True
            reset_pw = input('Do you want to reset the password? (y/N): ').strip().lower()
            if reset_pw == 'y':
