...
def test_buy_and_sell_signal_triggers_order_and_trade(mock_email, mock_telegram, mock_broker, clean_state_file):
    ...
    bot = BaseTradingBot({'trading_pair': 'BTCUSDT', 'initial_balance': 1000}, strategy, broker=mock_broker, paper_trading=False)
...
def test_broker_error_handling_and_notification(mock_email, mock_telegram, clean_state_file):
    ...
    bot = BaseTradingBot({'trading_pair': 'BTCUSDT', 'initial_balance': 1000}, strategy, broker=broker, paper_trading=False)
    bot.telegram_notifier = MagicMock()
    bot.email_notifier = MagicMock()
...
def test_state_persistence_and_recovery(mock_email, mock_telegram, mock_broker, clean_state_file):
    strategy = DummyStrategy()
    bot = BaseTradingBot({'trading_pair': 'BTCUSDT', 'initial_balance': 1000}, strategy, broker=mock_broker, paper_trading=False)
    bot.active_positions['BTCUSDT'] = {'entry_price': 100, 'size': 1, 'entry_time': '2024-01-01T00:00:00'}
    bot.trade_history.append({'pair': 'BTCUSDT', 'entry_price': 100, 'exit_price': 110, 'size': 1, 'pl': 10, 'time': '2024-01-01T01:00:00'})
    bot.current_balance = 1100
    bot.total_pnl = 10
    bot.save_state()
    # New bot loads state
    bot2 = BaseTradingBot({'trading_pair': 'BTCUSDT', 'initial_balance': 1000}, strategy, broker=mock_broker, paper_trading=False)
