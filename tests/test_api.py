...
def test_start_and_stop_bot():
    ...
    r = requests.post(f"{API_URL}/start_bot", json=payload, auth=HTTPBasicAuth(API_LOGIN, API_PASSWORD))
    ...
    r = requests.get(f"{API_URL}/trades", params={"bot_id": bot_id}, auth=HTTPBasicAuth(API_LOGIN, API_PASSWORD))
    assert r.status_code == 200
    assert isinstance(r.json(), list)

    # Stop bot
    r = requests.post(f"{API_URL}/stop_bot", json={"bot_id": bot_id}, auth=HTTPBasicAuth(API_LOGIN, API_PASSWORD))
    assert r.status_code == 200
