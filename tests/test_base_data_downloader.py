...
def test_download_multiple_symbols():
    ...
    try:
        downloader = DummyDownloader(data_dir=temp_dir, interval='1d')
        symbols = ['AAA', 'BBB']
        results = downloader.download_multiple_symbols(symbols, downloader.download_data, '2023-01-01', '2023-01-02')
...
def test_binance_data_downloader_integration(monkeypatch):
    ...
    try:
        ...
        bdd = BinanceDataDownloader(api_key='fake', api_secret='fake', data_dir=temp_dir, interval='1d')
        def fake_download(symbol, interval, start_date, end_date=None, save_to_csv=True):
            data = {
                'timestamp': pd.date_range(start=start_date, periods=2, freq='D'),
                'open': [1, 2],
                'high': [2, 3],
        ...
        results = bdd.download_multiple_symbols(symbols, '1d', '2023-01-01', '2023-01-02')
        assert set(results.keys()) == set(symbols)
        for path in results.values():
            assert os.path.exists(path)
    finally:
