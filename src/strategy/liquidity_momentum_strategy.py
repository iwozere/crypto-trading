...
class LiquidityMomentumStrategy(BaseStrategy):
    ...
    def next(self):
        ...
        if not self.position:
            # Entry Logic
            if (
                volume > vol_ma_val * self.params["volume_threshold"]
                and self.check_liquidity_conditions()
            ):

                self.log(
                    f"LONG ENTRY SIGNAL: Volume {volume:.2f} > MA {vol_ma_val:.2f}, Liquidity conditions met"
                )
