...
class RsiBbVolumeBot(BaseTradingBot):
    ...
    def run(self) -> None:
        ...
        data_feed = BinanceLiveFeed.from_binance(
            client, symbol=symbol, interval=interval, window=window
        )
...
if __name__ == "__main__":
    ...
    cerebro.setbroker(
        BinancePaperBroker(BINANCE_PAPER_KEY, BINANCE_PAPER_SECRET, cash=1000.0)
    )
    cerebro.run()
