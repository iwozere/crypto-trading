...
class EntryMixinFactory:
    ...
    def get_entry_mixin(entry_type: str):
        if entry_type not in EntryMixinFactory.ENTRY_LOGIC_REGISTRY:
            raise ValueError(f"Unknown entry logic: {entry_type}")
        return EntryMixinFactory.ENTRY_LOGIC_REGISTRY[entry_type]

    @staticmethod
    def get_entry_mixin_params(entry_type: str):
        if entry_type not in EntryMixinFactory.ENTRY_PARAM_MAP:
            raise ValueError(f"Unknown entry logic: {entry_type}")
        return EntryMixinFactory.ENTRY_PARAM_MAP[entry_type] 