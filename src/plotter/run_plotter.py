...
class ResultPlotter:
        ...
        for indicator in indicators:
            try:
                if indicator == "rsi":
                    ...
                    calculated_indicators["bollinger_bands"] = self._calculate_bollinger_bands(df["close"], period, std_dev)
                    ...
                    calculated_indicators["ichimoku"] = self._calculate_ichimoku(df, tenkan_period, kijun_period, senkou_span_b_period)
    ...
    def _calculate_ichimoku(self, df: pd.DataFrame, tenkan_period: int, kijun_period: int, senkou_span_b_period: int) -> Dict:
    ...
    def calculate_equity_curve(self, trades: List[Dict], initial_capital: float = 1000.0) -> Tuple[pd.Series, pd.Series]:
    ...
    def create_plot(self, df: pd.DataFrame, indicators: Dict, trades: List[Dict], 
                   equity_curve: pd.Series, json_file: str, result_data: Dict) -> None:
        ...
        fig, axes = plt.subplots(n_subplots, 1, figsize=plot_size, 
        ...
        fig.suptitle(f'Strategy: {strategy_name}\n{os.path.basename(json_file)}', 
        ...
        for indicator_name, indicator_data in indicators.items():
            if subplot_layout.get(indicator_name) == "overlay":
                if indicator_name == "bollinger_bands":
                    ax_price.plot(df.index, indicator_data["upper"], '--', label='BB Upper', alpha=0.7)
                    ax_price.plot(df.index, indicator_data["middle"], '-', label='BB Middle', alpha=0.7)
                    ax_price.plot(df.index, indicator_data["lower"], '--', label='BB Lower', alpha=0.7)
                    ...
                    ax_price.plot(df.index, indicator_data["senkou_span_a"], label='Senkou Span A', alpha=0.7)
                    ax_price.plot(df.index, indicator_data["senkou_span_b"], label='Senkou Span B', alpha=0.7)
            ...
            if subplot_layout.get(indicator_name) == "separate":
                if subplot_idx < len(axes):
                    ...
                    if indicator_name == "rsi":
                        ...
                        ax.bar(df.index, indicator_data, label='Volume', alpha=0.7, color='blue')
        ...
        if self.vis_settings.get("show_equity_curve", True) and not equity_curve.empty:
            ax_equity = axes[-1]
            ax_equity.plot(equity_curve.index, equity_curve.values, label='Equity Curve', color='green', linewidth=2)
        ...
        plt.savefig(output_file, dpi=self.vis_settings.get("plot_dpi", 300), 
                   bbox_inches='tight')
        plt.close()
    ...
    def _plot_trades(self, ax, trades: List[Dict]) -> None:
        ...
        for trade in trades:
            if trade.get('entry_time') and trade.get('exit_time'):
                ...
                ax.scatter(entry_time, entry_price, color='green', marker='^', s=100, 
                          label='Buy' if trade == trades[0] else "", zorder=5)
                
                # Plot sell marker (red triangle)
                ax.scatter(exit_time, exit_price, color='red', marker='v', s=100,
                          label='Sell' if trade == trades[0] else "", zorder=5)
