...
from src.analyzer.tickers_list import (get_all_us_tickers, get_six_tickers,
                                       get_sp500_tickers_wikipedia,
                                       get_sp_midcap_wikipedia)
...
from src.optimizer import (bb_volume_supertrend_optimizer,
                           ichimoku_rsi_volume_optimizer,
                           rsi_bb_volume_optimizer)
        ...
        bots_status.append(
            {
                ...
                "active_positions": len(
                    getattr(get_trades(bot_id), "active_positions", [])
                ),
    ...
    if request.method == "POST":
        ...
        if fundamentals and df is not None and not df.empty:
            try:
                ...
                fig.update_layout(
                    title=f"{symbol} - Last 1000 Candles with Indicators",
                    xaxis_title="Date",
                    yaxis_title="Price",
...
def twofa_setup():
    ...
    if not user.twofa_secret:
        ...
        user.twofa_secret = secret
        db.session.commit()
    otp_uri = pyotp.totp.TOTP(user.twofa_secret).provisioning_uri(
        name=user.email, issuer_name="CryptoTradingWebGUI"
    )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            filtered = [
                ...
                and params["min_price"] <= s["price"] <= params["max_price"]
                and s["volume"] is not None
                and s["volume"] >= params["min_volume"]
                and s["market_cap"] is not None
                and s["market_cap"] >= params["min_market_cap"]
                and (s["pe"] is None or s["pe"] <= params["max_pe"])
                and (s["roe"] is None or s["roe"] >= params["min_roe"])
                and (s["de"] is None or s["de"] <= params["max_de"])
                and (
                    not params["fcf_positive"]
                    or (s["fcf"] is not None and s["fcf"] > 0)
                )
                and (
                    not params["price_above_50d_200d"]
                    or (
                        s["price"] > (s["fifty_day"] or 0) > (s["two_hundred_day"] or 0)
                    )
                )
            ]
            results = filtered
            ...
            if "export" in request.form:
                ...
                return app.response_class(
                    ...
                    mimetype="text/csv",
                    headers={
                        "Content-Disposition": "attachment;filename=screened_stocks.csv"
                    },
                )
        except Exception as e:
            error = str(e)
